...
public class SimpleHoleTest : MonoBehaviour
{
    [Header("主Sprite渲染器")]
    ...
    private void FillCircleWithTransparency(Vector2 centerPixel, float radiusPixels)
    {
        ...
        // 计算位置补偿
        Vector3 positionOffset = CalculatePositionOffset(originalSprite, newSprite);
        mainSpriteRenderer.transform.position += positionOffset;
    ...
    private Vector3 CalculatePositionOffset(Sprite original, Sprite newSprite)
    {
        // 计算原始Sprite的中心点在世界空间中的位置
        Vector3 originalCenter = GetSpriteWorldCenter(original);
    
        // 临时设置新Sprite来计算中心点
        mainSpriteRenderer.sprite = newSprite;
        Vector3 newCenter = GetSpriteWorldCenter(newSprite);
    
        // 恢复原始Sprite
        mainSpriteRenderer.sprite = original;
    
        // 返回需要补偿的偏移量
        return originalCenter - newCenter;
    }

    private Vector3 GetSpriteWorldCenter(Sprite sprite)
    {
        ...
        // 转换到世界空间
        return mainSpriteRenderer.transform.TransformPoint(localCenter);
    }
    ...
    // 简单测试挖洞功能
    private void CutFixedRadiusCircle(Vector3 centerPos)
    {
        // 获取圆心在世界空间中的位置
        Vector3 worldCenter = centerPos;
        ...
        // 关键修复：考虑圆形的缩放比例
        Vector3 circleScale = new Vector3(0.03f, 0.03f, 0.03f); // 使用世界缩放
        float radiusWorld = 7.17f;
        float radiusPixels = radiusWorld * pixelsPerUnit * Mathf.Max(circleScale.x, circleScale.y) / Mathf.Max(localScale.x, localScale.y);
